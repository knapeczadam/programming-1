...
Ball::Ball(const Point2f& center, const Vector2f& velocity, const Color4f& color, float radius) : m_Center{center}
                                                                                                 , m_Velocity{velocity}
                                                                                                 , m_Color{color}
                                                                                                 , m_Radius{radius}
{
...
void Ball::Draw()
{
    utils::SetColor(Color4f{m_Color.r, m_Color.g, m_Color.b, m_Color.a});
...
void Ball::FillEllipse(const Point2f& center, float radX, float radY)
{
    const float dAngle{radX > radY ? float(M_PI / radX) : float(M_PI / radY)};

    glBegin(GL_POLYGON);
    {
        for (float angle = 0.0; angle < float(2 * M_PI); angle += dAngle)
        {
            glVertex2f(center.x + radX * float(cos(angle)), center.y * float(sin(angle)));
        }
    }
    glEnd();
}
