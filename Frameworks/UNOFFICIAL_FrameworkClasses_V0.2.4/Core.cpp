...
Core::Core(const Window& window) : m_Window{window}
                                   , m_Initialized{false}
                                   , m_pWindow{nullptr}
                                   , m_pContext{nullptr}
...
void Core::Initialize()
{
	...
	// Set the swap interval for the current OpenGL context, syncronize it with the vertical retrace
	...
	// Set the two-dimensional orthographic viewing region
	...
	{
		std::cerr << "Core::Initialize(), error when calling IMG_Init: " << SDL_GetError() << std::endl;
		return;
	}
	...
	if (TTF_Init() == -1)
	{
		std::cerr << "Core::Initialize(), error when calling TTF_Init: " << SDL_GetError() << std::endl;
		return;
	}
	...
	{
		std::cerr << "Core::Initialize(), error when calling Mix_OpenAudio: " << SDL_GetError() << std::endl;
		return;
	}
