...
Texture::Texture(const std::string& imagePath) : m_Id{}
                                                 , m_Width{10.0f}
                                                 , m_Height{10.0f}
                                                 , m_CreationOk{false}
{
...
Texture::Texture(const std::string& text, TTF_Font* pFont, const Color4f& textColor) : m_Id{}
	, m_Width{10.0f}
	, m_Height{10.0f}
	, m_CreationOk{false}
{
...
void Texture::CreateFromImage(const std::string& path)
{
	...
	{
		std::cerr << "Texture::CreateFromImage, error when calling IMG_Load: " << TTF_GetError() << std::endl;
		m_CreationOk = false;
		return;
	}
...
void Texture::CreateFromString(const std::string& text, TTF_Font* pFont, const Color4f& color)
{
	...
	if (pFont == nullptr)
	{
		std::cerr << "Texture::CreateFromString, invalid TTF_Font\n";
		m_CreationOk = false;
		return;
	}
...
void Texture::CreateFromSurface(SDL_Surface* pSurface)
{
	...
	case 4:
		...
		break;
	default:
		std::cerr << "Texture::CreateFromSurface, unknow pixel format, BytesPerPixel: " << pSurface->format->Rmask
			<< std::endl;
		m_CreationOk = false;
		return;
	}
	...
	// Select (bing) the texture we just generated as the current 2D texture OpenGL is using/modifying.
	// All subsequent changes to OpenGL's texturing state for 2D textures will affect this texture.
	glBindTexture(GL_TEXTURE_2D, m_Id);
	// check for errors, Can happen if a texture is created while a static pointer is being initialized
	GLenum e = glGetError();
	if (e != GL_NO_ERROR)
	{
		std::cerr << "Texture::CreateFromSurface, error binding textures, Error id = " << e << '\n';
		std::cerr << "Can happen if a texture is created before performing the initialization code (";
...
void Texture::Draw(const Rectf& dstRect, const Rectf& srcRect) const
{
	...
	float vertexRight{};
	float vertexTop{};
	if (!(dstRect.width > epsilon && dstRect.height > epsilon)) // If no size specified use default
	{
	...
	// Tell OpenGL which texture will use
	glBindTexture(GL_TEXTURE_2D, m_Id);
