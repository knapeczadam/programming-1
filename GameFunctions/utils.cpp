...
#pragma region textureImplementations
	...
	bool TextureFromString(const std::string& text, const std::string& fontPath, int ptSize, const Color4f& textColor,
	                       Texture& texture)
	{
		// Create font
		TTF_Font* pFont{};
	...
	void TextureFromSurface(const SDL_Surface* pSurface, Texture& texture)
	{
		...
		case 4:
			...
			break;
		default:
			std::cerr << "TextureFromSurface error: Unknow pixel format, BytesPerPixel: " << pSurface->format->
...
#pragma region CollisionFunctionality
	...
	bool IsPointInCircle(const Point2f& mousePos, const Circlef& circle)
	{
		return GetDistance(mousePos.x, mousePos.y, circle.center.x, circle.center.y) <= circle.radius;
	}

	bool IsPointInRect(const Point2f& mousePos, const Rectf& rect)
	{
		return (rect.left <= mousePos.x && mousePos.x <= rect.left + rect.width && rect.bottom <= mousePos.y && mousePos
			.y <= rect.bottom + rect.height);
	}

	bool IsOverlapping(const Rectf& r1, const Rectf& r2)
	{
		const Point2f topRightR1{r1.left + r1.width, r1.bottom + r1.height};
		const Point2f topRightR2{r2.left + r2.width, r2.bottom + r2.height};
		return r1.left <= topRightR2.x && r1.bottom <= topRightR2.y && r2.left <= topRightR1.x && r2.bottom <=
			topRightR1.y;
	}
