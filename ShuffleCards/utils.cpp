...
#pragma endregion CollisionFunctionality1
	...
	void Swap(Texture* arr, int idx1, int idx2)
	{
		//std::swap(arr[idx1], arr[idx2]);
		Texture temp{ std::move(arr[idx1])};
		arr[idx1] = std::move(arr[idx2]);
		arr[idx2] = std::move(temp);
	}

	void Shuffle(Texture* arr, int n, int swaps)
	{
		for (int i{}; i < swaps; ++i)
		{
